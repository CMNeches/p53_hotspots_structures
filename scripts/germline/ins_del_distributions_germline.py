...
plt.title("Base Pair Deletions in Germline Mutations of TP53" + " " + short_topo + " "  + bio_sex)
...
logbins = np.logspace(np.log10(bins[0]), np.log10(bins[-1]))
plt.hist(data, bins=logbins, ec = 'k')
plt.title("Base Pair Insertions in Germline Mutations of TP53" + " " + short_topo + " " + bio_sex)
...
if len(c_description) == 0:
    ...
    logbins = np.logspace(np.log10(bins[0]), np.log10(bins[-1]))
    plt.hist(data, bins = logbins, ec = 'k')
    plt.title("Frameshift Mutations in Germline Mutations of TP53" + " " + short_topo + " " + bio_sex)
