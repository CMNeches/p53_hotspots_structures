...
labels = ["Missense", "Splice", "Nonsense", "FS", "Other", "Large Del", "Intronic", "Silent"]
sizes = [3107, 386, 378, 350, 95, 78, 30, 14]
plt.figure()
plt.pie(sizes, labels=labels, autopct = '%1.1f%%', startangle=90)
#plt.show()

#make all of the graphs for the different types
codon_array = np.arange(1,394) #makes an array for 1 through 393 so that computer can count faster
...
for t in types:
    ...
    all_codons = temporary_df["Codon_number"].to_numpy() #helps us count
    mutation_counts = np.array([(all_codons == c).sum() for c in codon_array]) #.sum() is to leverage the Booleans; list comprehension
    plt.figure()

    #bar graph counting how many of each type of mutation there are (color coded by domain)
    ...
    plt.xlabel("TAD1 TAD2 PR DBD OD C-terminus \n Codon Position")
    plt.ylabel("Mutation Counts")
    plt.yscale("log")
    #plt.show()

    #frequencies- various "mutation_counts" terms will be one, so these show the entire data set  
    plt.bar(codon_array[1:41], mutation_counts[1:41]/mutation_counts.sum(), color = 'b')
    plt.bar(codon_array[41:62], mutation_counts[41:62]/mutation_counts.sum(), color = 'r')
    plt.bar(codon_array[65:93], mutation_counts[65:93]/mutation_counts.sum(), color = 'g')
    plt.bar(codon_array[101:301], mutation_counts[101:301]/mutation_counts.sum(), color = 'c')
    plt.bar(codon_array[324:356], mutation_counts[324:356]/mutation_counts.sum(), color = 'm')
    plt.bar(codon_array[365:394], mutation_counts[365:394]/mutation_counts.sum(), color = 'y')
    plt.title("Codon Position vs. Frequency of Mutation in " + t + " Germline Mutations of TP53")
    plt.xlabel("Codon Position")
    plt.ylabel("Frequency")
    #plt.show()

    #per domain graphs- individual hotspots in the regions, normalized by region (each sub-figure add up to 1)
